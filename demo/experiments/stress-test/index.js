                ...
                if(toForward) {
                    ...
                    if(tileKeyframes.length) {
                        lastTileKeyframes = tileKeyframes.filter(function(keyframe) {
                            return (
                                (tileKeyframes[tileKeyframes.length - 1].time === keyframe.time) &&
                    ...
                    if(changeKeyframes.length) {
                        lastTileKeyframes = this._keyframes.filter(function(keyframe) {
                            return (
                                (changeKeyframes[0].index === keyframe.index) &&
                                (keyframe.type !== 'change') &&
                                (keyframe.time < changeKeyframes[0].time)
                            );
                        });
                    }
                }
